...
func (s *triggerSubscriber) RegisterTrigger(ctx context.Context, request commoncap.TriggerRegistrationRequest) (<-chan commoncap.TriggerResponse, error) {
	...
	// Wait for the registration status with a timeout.  Always return the response channel to the caller even if an
	// error occurs. The caller must decide whether the error is transient and whether to continue listening on the
	// channel for events, as registration may succeed on a subsequent attempt.
	...
	if err != nil {
		...
		return reg.GetTriggerResponseChannel(), registration.ErrUnableToDetermineRegistrationStatus
	}
	defer subCancel()
	...
	if errors.Is(regErr, registration.ErrUnableToDetermineRegistrationStatus) {
		...
		s.lggr.Infow("registration error occurred", "donId", cfg.capDonInfo.ID, "workflowID", request.Metadata.WorkflowID, "error", regErr)
	}
